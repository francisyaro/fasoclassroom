        ...
        if (p && p.completedLessons.length > 0 && !p.completed) {
            ...
            activeItem.innerHTML = `
                ...
                <div style="display: flex; align-items: center; gap: 1rem; justify-content: space-between; flex-wrap: wrap;">
                    ...
                    ${store.hasPaid() ? 
                        `<span style="color: #25d366; font-size: 0.8rem; font-weight: 600; display: flex; align-items: center; gap: 0.25rem;">✅ Payé & Débloqué</span>` : 
                        `<span style="color: #ff9800; font-size: 0.8rem; font-weight: 600; display: flex; align-items: center; gap: 0.25rem;">🔒 Paiement Requis</span>`
                    }
                </div>
            `;
        ...
        card.innerHTML = `
            ...
            <div style="display: flex; gap: 0.5rem; margin-bottom: 0.8rem; align-items: center; flex-wrap: wrap;">
                ...
                ${currentUser && currentUser.role === 'Étudiant' ? (
                    store.hasPaid() ? 
                    `<span class="course-badge" style="background: rgba(37, 211, 102, 0.1); color: #25d366; border: 1px solid rgba(37, 211, 102, 0.25); border-radius: 4px; padding: 0.2rem 0.5rem; font-size: 0.7rem; font-weight: 600; display: inline-flex; align-items: center;">🔓 Débloqué</span>` : 
                    `<span class="course-badge" style="background: rgba(255, 152, 0, 0.1); color: #ff9800; border: 1px solid rgba(255, 152, 0, 0.25); border-radius: 4px; padding: 0.2rem 0.5rem; font-size: 0.7rem; font-weight: 600; display: inline-flex; align-items: center;">🔒 Paiement Requis</span>`
                ) : ''}
            </div>
